        ...
        stage('SonarQube Scan') {
            steps {
                ...
                withSonarQubeEnv(env.SONARQUBE_SERVER_ID) {
                    sh "mvn org.sonarsource.scanner.maven:sonar-maven-plugin:sonar -Dsonar.host.url=${env.SONAR_HOST_URL} -Dsonar.token=${env.SONAR_TOKEN} -Dsonar.projectKey=${env.JOB_NAME} -Dsonar.sources=."
